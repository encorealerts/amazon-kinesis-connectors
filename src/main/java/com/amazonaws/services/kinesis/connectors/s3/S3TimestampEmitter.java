 ...
 */
public class S3TimestampEmitter implements IEmitter<byte[]> {
    private static final Log LOG = LogFactory.getLog(S3TimestampEmitter.class);
    protected final String s3Bucket;
    protected final String s3Endpoint;
    protected final AmazonS3Client s3Client;

    public S3TimestampEmitter(KinesisConnectorConfiguration configuration) {
    ...
    protected String getS3FileName(String firstSeq, String lastSeq) {
        return new SimpleDateFormat("yyyyMMddHH").format(Calendar.getInstance().getTime()) + "/" + firstSeq + "-" + lastSeq;
    }
